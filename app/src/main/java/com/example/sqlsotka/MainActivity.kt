...
import com.example.sqlsotka.databinding.ActivityMainBinding
...
class MainActivity : AppCompatActivity() {
    ...
    fun saveRecord(view: View){
        ...
        val id = bindingClass.uId.text.toString()
        val name = bindingClass.uName.text.toString()
        val email = bindingClass.uEmail.text.toString()
        val databaseHandler = DatabaseHandler(this)
        if(id!="" && name!="" && email!=""){
            val status = databaseHandler.addEmployee(EmpModelClass(Integer.parseInt(id),name, email))
            if(status > -1){
                Toast.makeText(applicationContext,"record save",Toast.LENGTH_LONG).show()
                bindingClass.uId.text.clear()
                bindingClass.uName.text.clear()
                bindingClass.uEmail.text.clear()
            }
        }else{
    ...
    fun viewRecord(view: View){
        ...
        bindingClass.listView.adapter = myListAdapter
    }
